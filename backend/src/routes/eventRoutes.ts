...
router.post(
  ...
  authenticate,
  authorize("admin"),
  eventController.upload.single("thumbnail_image"),
  [
    body("title").trim().notEmpty().withMessage("Judul event wajib diisi"),
    body("description")
      .trim()
      .notEmpty()
      .withMessage("Deskripsi event wajib diisi"),
    body("date")
    ...
    body("meeting_url")
      .optional({ nullable: true })
      .isURL()
      .withMessage("Meeting URL tidak valid"),
    body("published")
      .isBoolean()
      .withMessage("Published wajib diisi (boolean)"),
  ],
