...
export const getCourseStats = async (courseId: number, mentorId: number) => {
    const course = await Course.findOne({
        where: { id: courseId, is_deleted: false },
    });
    ...
    if (course.mentor_id !== mentorId) {
        throw new Error('Anda tidak memiliki akses ke statistik kursus ini');
    }
    ...
    const [completionResult] = await sequelize.query(
    ...
    const activeStudents =
        parseInt((activeResult as any)[0].active_students) || 0;
    const avgProgress =
        parseFloat((completionResult as any)[0].avg_progress) || 0;

    return {
        activeStudents,
        completionRate: Math.round(avgProgress * 100) / 100,
    };
};
