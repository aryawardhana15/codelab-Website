...
export const getMyEnrolledCourses = async (
    ...
    try {
        ...
        const courses = await courseService.getMyEnrolledCourses(
            req.user.userId,
        );
...
export const getCourseStats = async (
    ...
    try {
        ...
        res.status(200).json({
            success: true,
            message: 'Course stats retrieved',
            data: stats,
        });
    } catch (error: any) {
        const status = error.message?.includes('akses') ? 403 : 404;
        res.status(status).json({
            success: false,
            message: error.message || 'Failed to get course stats',
        });
    }
};
