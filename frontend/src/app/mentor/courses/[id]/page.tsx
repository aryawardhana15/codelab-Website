...
export default function MentorCourseDashboard() {
    ...
    const fetchCourseData = async () => {
        try {
            ...
            // Mock stats fetch - in real app would need an endpoint
            // const statsRes = await api.get(`/courses/${courseId}/stats`);
            setStats({
                totalStudents: 120, // Mock
                activeStudents: 85, // Mock
            ...
            <ProtectedRoute allowedRoles={['mentor']}>
                <div className="min-h-screen bg-light-50">
                    ...
                    <div className="flex flex-col justify-center items-center h-[calc(100vh-64px)]">
                        ...
                        <p className="text-gray-500 font-medium">Memuat dashboard kursus...</p>
                    </div>
                </div>
            </ProtectedRoute>
        );
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
                            <div>
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600">
                                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold border ${course?.difficulty === 'beginner' ? 'bg-green-100 text-green-700 border-green-200' :
                                    ...
                                    <Link href={`/courses/${courseId}`} className="flex items-center gap-1 text-primary hover:underline">
                                        Isi Preview <ExternalLink className="w-3 h-3" />
                                    </Link>
                                </div>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {menuItems.map((item, index) => (
                            <Link
                                ...
                                key={index}
                                className={`group relative p-6 rounded-2xl border transition-all duration-300 ${item.disabled
